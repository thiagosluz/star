...
> **Estado:** FASES 1 a 17, 21 a 25, 29 a 47 concluídas (F15, F21, F22, F29–F47 entregues; a F48+ é a próxima) · **2351 testes** unitários/integração · **159 testes E2E**
...
> · **equipe do evento na página pública** (bloco que o organizador adiciona ou não, com o corpo
> lido das equipes reais — nome e equipe são do evento, foto e contato são da pessoa)
> · **toda imagem enviada é gravada em WebP** (convertida no servidor, com o original apagado, os
> dados da câmera fora e logotipo sem perda) e **a organização publica a foto do palestrante que
> não tem conta**, com declaração de autorização na trilha e a pessoa podendo trocá-la ou removê-la
...
| Domínio | Capacidade |
|---|---|
| **Multi-tenancy + RBAC** | Uma base, várias instituições isoladas por Row-Level Security; 11 papéis e 66 permissões, com acúmulo de papéis e troca de contexto sem perder a sessão |
...
| **Catálogo de gamificação** | Carta e missão **editáveis e excluíveis** pela tela (a exclusão é **lógica**, com trilha). Excluir carta a tira do catálogo e **para de concedê-la**, mas ela **continua no álbum de quem a ganhou** — conquista não se apaga; e a carta que é **prêmio de missão ou de QR de patrocinador** recusa a exclusão com a contagem, porque a promessa em vigor não pode ficar sem prêmio. A missão excluída sai das duas listas e **preserva progresso e XP resgatado**, com o número dito no diálogo. E os fatos que a gamificação não enxergava passaram a valer: **inscrição confirmada** (30 XP, nas três portas — inscrição, promoção da lista de espera e confirmação de vaga), **certificado emitido** (50 XP por documento), **sorteio ganho** (0 XP e a carta de "sorteado": o prêmio já é a recompensa) e a **proposta de chamada**, que agora paga o mesmo que o artigo ao enviar e ao ser aceita. O formulário de missão só oferece as origens que **algum caminho do sistema emite** |
| **Perfil público do participante** | A pessoa ganha **endereço próprio** (`/t/<slug>/u/<handle>`) e decide **o que** aparece e **para quem** — **dezesseis campos**, cada um com três níveis (`qualquer pessoa na internet` · `só quem participa desta instituição` · `só eu`), com padrão conservador: identidade, nível, cartas em destaque, certificados, interesses, links e contagem de eventos são públicos; **XP, dias seguidos, coleção, lista de eventos e posição relativa** só para quem participa; **e-mail e redes sociais (LinkedIn, Instagram, GitHub e YouTube) nascem FECHADOS**; **diretório e buscadores nascem desligados**. O pacote é montado **campo a campo** com allowlist testada, e **perfil todo privado responde 404** (nunca "existe, mas você não pode ver"). A página **só existe onde a pessoa participa** — o `user` é global e a RLS não o protege, então o `@handle` não é janela para dentro de casa alheia. O `@handle` é **global**, único sem diferenciar maiúscula, com reservadas (rotas da plataforma) e **30 dias entre trocas**; publicar **não dá XP** (consentimento não é pedágio) e a trilha guarda a **decisão** (quais campos foram divulgados), nunca a bio. A mesma tela quita a **E35**: o nome completo no resultado público do sorteio passa a ser uma escolha |
| **Equipe do evento na página pública** | O organizador ganha o bloco **"Equipe do evento"** e escolhe **se ele entra ou não** na página — o corpo é a **equipe real do evento** (as mesmas equipes que organizam as demandas internas, FASE 38), lida na renderização: quem sai da equipe sai da página. Cada cartão traz a foto (ou as **iniciais**), o nome e a **etiqueta do nome da equipe** embaixo da foto — "Presidente", "Programação", "Logística e Infraestrutura / Mostra" quando a pessoa está em duas —, com o **líder primeiro** e ordem estável. **Nome e equipe são informação do evento; foto e contato são da pessoa**: e-mail, LinkedIn, Instagram, GitHub e YouTube só saem quando ela autoriza o campo **"E-mail e redes sociais"** no próprio perfil (que nasce desligado), e o mesmo interruptor acende o contato **no perfil e no cartão da equipe**. Equipe desativada não aparece, e **quem perdeu o vínculo com a instituição sai da vitrine** |
| **Toda imagem enviada vira WebP** | O arquivo entra no formato que o organizador tem (PNG, JPEG, WebP ou AVIF) e é **guardado em WebP**, convertido **no servidor** na confirmação do upload — o cliente não decide o formato do acervo, e o objeto enviado é **apagado** depois. A codificação é por finalidade: **foto** com perda calibrada e teto de pixels (foto do palestrante 512 px, capa 1920 px, galeria 2560 px — nunca amplia) e **logotipo sem perda**, porque marca tem borda de letra. Os **metadados da câmera (data e local) não são publicados** e a orientação do EXIF é assentada nos pixels. A tela mostra o que ficou gravado e **quanto economizou** ("620 KB — 74% menor"), e o acervo mostra o formato de cada imagem. A conversão é também a **validação de conteúdo**: o arquivo é decodificado, então HTML disfarçado de PNG não passa (e o objeto é removido). Imagem **animada** é recusada com o motivo, em vez de achatada num quadro, e uma imagem que declare pixels demais é recusada pelo cabeçalho, antes de alocar memória. A **arte de fundo do certificado continua JPEG** — PDF não embute WebP |
| **Foto do palestrante sem conta** | Quem vem de fora costuma nunca entrar na plataforma — e até aqui isso significava vitrine sem foto, sem que a organização pudesse resolver. Agora a organização **envia a foto** no cadastro **e na edição** da ficha (que passou a existir), marcando a **declaração de autorização**, exigida pelo serviço e registrada na **trilha** com autor e hora. A **origem da foto é dado** (`ORGANIZATION` × `SPEAKER`): o portal avisa quando a imagem não foi a pessoa que enviou e oferece as duas saídas — **trocar** ou **remover** (a remoção passou a existir nos dois lados). Corrigir o nome **não apaga** a foto (campo ausente preserva; campo vazio apaga), e a ficha mostra "Foto publicada pela organização" enquanto a vitrine exibe a imagem como qualquer outra |
| **Minha conta** | Área **global** (`/conta`) — a identidade vale em qualquer instituição e existe mesmo para quem não tem vínculo. Reúne **dados** (nome; e-mail com **confirmação no endereço novo** e a senha atual como prova; **foto**), **senha** (trocar com a atual — encerrando as outras sessões — ou **criar**, para quem entrou por convite e nunca teve senha), **segundo fator** (aplicativo autenticador com QR gerado no servidor, **10 códigos de recuperação** de uso único, desligamento exigindo a senha) e **dispositivos conectados** (navegador e sistema legíveis, "este dispositivo", encerrar uma ou todas as outras) |
| **Esqueci minha senha** | Link na tela de entrada, **página de pedido** e **página de nova senha** — o envio existia desde a FASE 15, mas o link do e-mail caía em 404: não havia tela nenhuma. A resposta ao pedido é **a mesma exista ou não a conta** (não é oráculo de quem tem cadastro), o link vale 60 minutos e só serve uma vez, e a redefinição **encerra as sessões abertas**. Com o segundo fator ligado, entrar passa a exigir o código do aplicativo **ou um código de recuperação**, no mesmo campo |
...
| **Comunicação** | **E-mail transacional de verdade** (Resend) com fila, tentativas e um **outbox** (`email_messages`) que guarda o que foi enviado antes de sair: assunto, HTML e texto, destinatário, situação, driver e o motivo da falha. **Vinte e duas** mensagens em templates puros — confirmação de e-mail, **troca de e-mail**, redefinição de senha, **convite de equipe**, avaliação atribuída, prazo de parecer (aviso e vencimento), carta conquistada, certificado emitido, **recado ao participante**, **proposta recebida**, **convite de palestrante** (com o link do portal e validade de 14 dias), os cinco avisos de **confirmação de vaga**, a **decisão da proposta** e os quatro de **demanda interna**. A instituição **convida a própria equipe** pela tela de equipe (o convite não ocupa vaga no plano; o vínculo nasce no aceite, onde a quota é conferida), acompanha tudo em **Comunicação** e a pessoa confirma o próprio endereço numa página própria. **O padrão é não enviar**: sem `EMAIL_DRIVER=resend` declarado, o e-mail é registrado e não sai da máquina |
...
```bash
npm test                  # 2351 testes (106 arquivos) — unit + integração com banco real
npm run test:e2e          # 159 testes E2E contra o container de produção
...
| [`docs/fase-02-auth-rbac.md`](docs/fase-02-auth-rbac.md) | Better Auth, 11 papéis (66 permissões hoje, incluindo as de plataforma), escopos, acúmulo de papéis, troca de contexto por cookie assinado | ADR-009 … 013 |
...
| [`docs/dividas-tecnicas.md`](docs/dividas-tecnicas.md) | **Levantamento consolidado:** 64 dívidas abertas (o levantamento original mais o que cada fase declarou), verificadas no código, por tema, com esforço e fases candidatas numeradas como as fases que serão entregues | — |
...
| [`docs/fase-43-catalogo-de-gamificacao.md`](docs/fase-43-catalogo-de-gamificacao.md) | **Catálogo de gamificação:** auditoria dos gatilhos (o que faltava e o que era promessa falsa), **editar** e **excluir** carta e missão — exclusão lógica, com guarda de uso na carta (prêmio de missão/QR recusa) e o **álbum de quem ganhou preservado** — e os fatos que não moviam nada: **inscrição confirmada** (30 XP nas três portas, com a chave de idempotência no ALVO para matar o farm), **certificado emitido** (50 XP na geração, por onde passam a tela e o worker), **sorteio ganho** (0 XP + carta, só o ganhador) e a **proposta de chamada** (enviar e aceitar, com a mesma chave do comitê); inclui o defeito que deixava **criar missão pela tela impossível** com minutos em branco | ADR-233 … 235 |
| [`docs/fase-45-equipe-na-pagina-publica.md`](docs/fase-45-equipe-na-pagina-publica.md) | **Equipe do evento na página pública:** bloco novo que o organizador adiciona **ou não**, com o corpo lido das **equipes reais do evento** (F38) — a etiqueta é o nome da equipe, o líder vem primeiro e quem está em duas equipes aparece uma vez com as duas. **Nome e equipe são do evento; foto e contato são da pessoa**: e-mail, LinkedIn, Instagram, GitHub e YouTube saem só com o campo **"E-mail e redes sociais"** autorizado (nasce desligado), e o mesmo interruptor acende o contato no perfil e no cartão. Equipe desativada não aparece, **quem perdeu o vínculo sai da vitrine**, e o leitor de contatos é tolerante rede a rede | ADR-244 … 247 |
| [`docs/fase-46-webp-e-foto-do-palestrante.md`](docs/fase-46-webp-e-foto-do-palestrante.md) | **Imagens em WebP e a foto do palestrante sem conta:** toda imagem enviada é **reconvertida no servidor** para WebP na confirmação do upload — com o **original apagado**, política por finalidade (foto com perda calibrada e teto de pixels; logotipo **sem perda**), metadados da câmera descartados e a orientação do EXIF assentada — e a decodificação virou a **validação de conteúdo** (HTML disfarçado de PNG deixou de ser aceito e servido). Imagem animada é **recusada**, não achatada, e a bomba de pixels é recusada pelo cabeçalho; a arte do certificado fica de fora (PDF não embute WebP). Do outro lado, a **organização publica a foto do palestrante que não tem conta**, com declaração de autorização exigida pelo serviço e registrada na trilha, origem da foto em coluna e o portal avisando quando não foi a pessoa quem enviou — com **trocar** e **remover** nos dois lados, e a ficha do palestrante finalmente editável | ADR-248 … 253 |
| [`docs/fase-47-area-de-conta.md`](docs/fase-47-area-de-conta.md) | **Área de conta:** `/conta` como área **global** da identidade — **dados** (nome; e-mail com **confirmação no endereço novo** e a senha atual como prova; a **foto** que faltava, agora com escritor e convertida em WebP), **senha** (trocar encerrando as outras sessões, ou **criar** para quem entrou por convite e nunca teve uma), **segundo fator** TOTP com QR gerado no servidor, **10 códigos de recuperação** de uso único e desligamento exigindo a senha, e **dispositivos conectados** com encerramento seletivo; **esqueci minha senha** com link no login, página de pedido e página de nova senha — o envio existia desde a FASE 15 e o link do e-mail caía em **404**; **desafio de dois fatores no login**, com o contexto da instituição preservado para depois do código; e o segredo do TOTP numa tabela que a role de runtime **não alcança** (privilégio revogado, como `job_runs`). Três defeitos reais: o beco sem saída da redefinição, o **código de recuperação correto recusado** pela normalização que eu inventei, e o template novo fora da lista de testes (`tests/**` não passa pelo `typecheck`) | ADR-254 … 259 |
| [`docs/fase-44-perfil-publico-do-participante.md`](docs/fase-44-perfil-publico-do-participante.md) | **Perfil público do participante:** endereço próprio (`/u/<handle>`) com **quinze campos**, cada um com o próprio nível de visibilidade (internet · quem participa desta instituição · só eu), pacote montado **campo a campo** com allowlist testada, **404 para perfil todo privado** e página que **só existe onde a pessoa participa** (o `user` é global e a RLS não o protege); `@handle` global único sem caixa com reservadas e espera de 30 dias; **publicar não dá XP**; trilha guarda a decisão, não a bio; e a **E35** quitada (o nome no resultado público do sorteio virou escolha). Quatro defeitos reais encontrados: a **posição relativa invertida**, a página em casa alheia, os identificadores acadêmicos sem gravador e o rótulo duplicado na tela | ADR-236 … 243 |
...
14. **Imagens do evento: allowlist fechada (FASE 17) e formato de armazenamento único (FASE 46).**
    Capa e logotipos aceitam apenas PNG, JPEG, WebP e AVIF — SVG é recusado de propósito,
    porque um SVG pode conter script e seria executado no navegador do visitante. GIF e
    TIFF ficam de fora por decisão, não por esquecimento. Desde a FASE 46 o que se **guarda**
    é sempre WebP (o original é apagado depois da conversão), e a imagem **animada** é
    recusada em vez de achatada num quadro — a exceção é a arte de fundo do certificado,
    que continua JPEG porque o PDF a embute como `/DCTDecode` e **PDF não embute WebP**.
